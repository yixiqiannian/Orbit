"""
WeRead → Orbit 直连数据库同步脚本（不需要后端运行）

用法: G:\Python3.11\python.exe sync_weread_direct.py
功能: 直接调 WeRead API → 写入 MySQL，跳过 FastAPI 后端
适用: cron 无人值守、后端未启动、快速批量同步

区别于 sync_weread.py（需要后端运行 + JWT 认证）：
- 本脚本直连 MySQL，不依赖 FastAPI 后端
- 使用 pymysql + httpx，需要系统 Python（非 Hermes venv）
- 适合后端不便启动的场景
"""
import sys
import time
...
# ── WeRead API ──
WEREAD_API_KEY = "wrk-5DBk3Kd4RtiRuLlaVbvKFgAA"  # 从 backend/.env 获取
...
DB_CONFIG = {
    ...
    "port": 3306,
    "user": "root",
    "password": "Qq564725728.",  # 从 backend/.env 获取
    "database": "orbit",
    "charset": "utf8mb4",
}
...
def upsert_book(conn, weread_id, title, author, cover, progress, last_read_at, finish_reading):
    """Insert or update a book. Returns 'inserted' or 'updated'."""
